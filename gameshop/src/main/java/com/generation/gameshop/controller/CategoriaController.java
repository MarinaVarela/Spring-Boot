...
@RestController
@RequestMapping("/categoria") //chamar no postman
@CrossOrigin("*")
public class CategoriaController {
	...
	@GetMapping("/{id}")
	private ResponseEntity<CategoriaModel> getALL(@PathVariable Long id) {
		return repository.findById(id).map(resp -> ResponseEntity.ok(resp)).orElse(ResponseEntity.notFound().build());
	}

	@GetMapping("/categoriaModel/{categoriaModel}")
	private ResponseEntity<List<CategoriaModel>> getByCategoria(@PathVariable String categoria) {
